...
IPWS_BIN_$(d) := $(call go-curr-pkg-tgt)
...
$(d)-try-build $(IPWS_BIN_$(d)): GOFLAGS += $(cmd/ipws_flags)

# uses second expansion to collect all $(DEPS_GO)
$(IPWS_BIN_$(d)): $(d) $$(DEPS_GO) ALWAYS #| $(DEPS_OO_$(d))
	$(go-build-relative)
...
$(d)-install: GOFLAGS += $(cmd/ipws_flags)
$(d)-install: $(d) $$(DEPS_GO) ALWAYS 
	$(GOCC) install $(go-flags-with-tags) ./cmd/ipws
.PHONY: $(d)-install

COVER_BIN_$(d) := $(d)/ipws-test-cover
CLEAN += $(COVER_BIN_$(d))

$(COVER_BIN_$(d)): GOTAGS += testrunmain
$(COVER_BIN_$(d)): $(d) $$(DEPS_GO) ALWAYS
	$(eval TMP_PKGS := $(shell $(GOCC) list -f '{{range .Deps}}{{.}} {{end}}' $(go-flags-with-tags) ./cmd/ipws | sed 's/ /\n/g' | grep ipfs/go-ipfs) $(call go-pkg-name,$<))
