// cmd/ipws implements the primary CLI binary for ipws
package main
...
var log = logging.Logger("cmd/ipws")

// declared as a var for testing purposes
var dnsResolver = madns.DefaultResolver

const (
	EnvEnableProfiling = "IPWS_PROF"
	cpuProfile         = "ipws.cpuprof"
	heapProfile        = "ipws.memprof"
)
...
func mainRet() int {
	...
	// Handle `ipws version` or `ipws help`
	if len(os.Args) > 1 {
		// Handle `ipws --version'
		...
		//Handle `ipws help` and `ipws help <sub-command>`
		if os.Args[1] == "help" {
			if len(os.Args) > 2 {
				os.Args = append(os.Args[:1], os.Args[2:]...)
				// Handle `ipws help --help`
				// append `--help`,when the command is not `ipws help --help`
				if os.Args[1] != "--help" {
...
func checkDebug(req *cmds.Request) {
	...
	if debug || os.Getenv("IPWS_LOGGING") == "debug" {
		u.Debug = true
...
// command ought to be executed on an ipws daemon.
//
...
func commandShouldRunOnDaemon(details cmdDetails, req *cmds.Request, cctx *oldcmds.Context) (http.Client, error) {
	...
	if client != nil {
		if details.cannotRunOnDaemon {
			...
			if daemonLocked, _ := fsrepo.LockedByOtherProcess(cctx.ConfigRoot); daemonLocked {
				return nil, cmds.ClientError("ipws daemon is running. please stop it to run this command")
			}
			return nil, nil
		}
	...
	if details.cannotRunOnClient {
		return nil, cmds.ClientError("must run on the ipws daemon")
	}
...
var apiFileErrorFmt string = `Failed to parse '%[1]s/api' file.
	error: %[2]s
If you're sure go-ipws isn't running, you can just delete it.
`
var checkIPWSUnixFmt = "Otherwise check:\n\tps aux | grep ipws"
var checkIPWSWinFmt = "Otherwise check:\n\ttasklist | findstr ipws"
...
func getAPIClient(ctx context.Context, repoPath, apiAddrStr string) (http.Client, error) {
	...
	switch {
	case osh.IsUnix():
		apiErrorFmt = apiFileErrorFmt + checkIPWSUnixFmt
	case osh.IsWindows():
		apiErrorFmt = apiFileErrorFmt + checkIPWSWinFmt
	default:
