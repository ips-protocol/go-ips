# The go-ipws config file

The go-ipws config file is a JSON document located at `$IPWS_PATH/config`. It
...
applied with `--profile` flag to `ipws init` or with the `ipws config profile
apply` command. When a profile is applied a backup of the configuration file
will be created in `$IPWS_PATH`.
...
- `test`
  ...
  Reduces external interference, useful for running ipws in test environments.
...
- `badgerds`
  ...
  If you apply this profile after `ipws init`, you will need to convert your
  datastore to the new configuration. You can do this using [ipws-ds-convert](https://github.com/ipfs/ipfs-ds-convert)
...
Default: The ipws.top bootstrap nodes
...
- `StorageMax`
A soft upper limit for the size of the ipws repository's datastore. With `StorageGCWatermark`,
...
- `Spec`
Spec defines the structure of the ipws datastore. It is a composable structure, where each datastore is represented by a json object. Datastores can wrap other datastores to provide extra functionality (eg metrics, logging, or caching).

This can be changed manually, however, if you make any changes that require a different on-disk structure, you will need to run the [ipws-ds-convert tool](https://github.com/ipfs/ipfs-ds-convert) to migrate data into the new structures.
...
## `Discovery`
Contains options for configuring ipws node discovery mechanisms.
...
its merely here for convenience. Ipws will always generate the peerID from its
keypair at runtime.
...
A boolean value that when set to true, will cause ipws to not keep track of
