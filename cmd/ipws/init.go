...
var initCmd = &cmds.Command{
	Helptext: cmds.HelpText{
		Tagline: "Initializes ipws config file.",
		ShortDescription: `
Initializes ipws configuration files and generates a new keypair.

If you are going to run IPWS in server environment, you may want to
initialize it using 'server' profile.

For the list of available profiles see 'ipws config profile --help'
...
`,
	...
	PreRun: func(req *cmds.Request, env cmds.Environment) error {
		...
		if daemonLocked {
			log.Debug("ipws daemon is running")
			e := "ipws daemon is running. please stop it to run this command"
...
var errRepoExists = errors.New(`ipws configuration file already exists!
...
func doInit(out io.Writer, repoRoot string, empty bool, nBitsForKeypair int, confProfiles []string, conf *config.Config) error {
	if _, err := fmt.Fprintf(out, "initializing IPWS node at %s\n", repoRoot); err != nil {
		return err
	}
	...
	if conf == nil {
		...
		diskStat, err := disk.Usage(repoRoot)
		if err == nil {
			free := diskStat.Free / (1024 * 1024 * 1024)
			conf.Datastore.StorageMax = fmt.Sprintf("%vGB", free)
		}
	}
